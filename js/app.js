...
function cardBlog(b){
  ...
  <div class="card" data-id="${b.id}" data-type="blog" role="button" tabindex="0" aria-label="Read ${escapeHtml(b.title)}">
    ...
    <div class="card-body">
      <div class="card-meta">${renderMetaTags([b.category])}${b.author?`<span class="card-tag"><i class="fa fa-user"></i> ${escapeHtml(b.author)}</span>`:''}</div>
      <h3 class="card-title">${escapeHtml(b.title)}</h3>
      <p class="card-desc" style="font-size:.83rem;color:var(--text-secondary);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;margin:.35rem 0 0">${escapeHtml(b.shortDescription||b.short_description||'')}</p>
      <div class="card-details" style="margin-top:.5rem">
        ${b.date?`<span><i class="fa fa-calendar"></i> ${formatDate(b.date)}</span>`:''}
        ${b.readTime||b.read_time?`<span><i class="fa fa-clock"></i> ${escapeHtml(b.readTime||b.read_time)}</span>`:''}
      </div>
    </div>
    <div class="card-footer">
...
function openCardDetails(item, type) {
  ...
  const actions=[
    ...
    detailAction('Download',item.downloadLink||item.download_link,true),
